...
export const Tests: React.FC<TestsProps> = ({
  ...
  const handleRun = async (forceVariant?: 'solution') => {
    ...
    if (!trimmedCode) {
      pushConsoleError(t('testsWindow.emptyCodeError'))
      return
    }

    if (!isFunctionLike(trimmedCode)) {
      pushConsoleError(t('testsWindow.mustBeFunctionError'))
      return
    }
    ...
    try {
      ...
      if (variant === 'quickTest') {
        ...
        const quickResult: QuickResults = {
          ...
          yourResult: first.error
            ? t('testsWindow.errorPrefix', { error: first.error })
      ...
      const fullResult: FullResults = {
        kind: 'full',
        cases: response.tests.map((test, index) => ({
          id: String(index + 1),
          inputData: JSON.stringify(test.input, null, 2),
          expectedResult: JSON.stringify(test.expectedResult),
          yourResult: test.error
            ? t('testsWindow.errorPrefix', { error: test.error })
            : JSON.stringify(test.codeOutcome),
          passed: test.testOutcome,
        })),
      }
      ...
      const message =
        error instanceof Error ? error.message : t('testsWindow.genericError')
